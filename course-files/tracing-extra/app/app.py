...
@app.route("/alpha")
def alpha():
    with tracer.start_span('TestAlphaSpan') as span:
        for i in range(100):
            span.set_tag('do_heavy_work;', do_heavy_work)  # removed the colon here since it caused a syntax error - not sure about its purpose?
            if i % 100 == 99:
                time.sleep(10)
...
@app.route("/beta")
def beta():
    with tracer.start_span('TestBetaSpan') as span:
